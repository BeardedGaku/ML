function avg_error = kFoldValidationNewClassifier(data1,data2,k)
...
for i=1:k 
   ...
   Xtrain2 = cat(1,data2(1:(testPoints*(i-1)),:),data2(((testPoints*(i)+1):(s)),:));
    ...
    errorNC = 0;
    for j=1:testPoints
        cn = new_classifier(data1(j + testPoints*(i-1),:),mu1MLE,mu2MLE);        
        errorNC = errorNC + (cn ~= 1);
        cn = new_classifier(data2(j + testPoints*(i-1),:),mu1MLE,mu2MLE);
        errorNC = errorNC + (cn ~= -1);
    end
